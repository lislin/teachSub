...
public class SelectServerSocket {
	public static int PORT_NUMBER = 8888;

	public static void main(String[] argv) throws Exception {
		new SelectServerSocket().go(argv);
	}
	...
	public void go(String[] argv) throws Exception {
		...
		while (true) {
			...
			java.util.Iterator<SelectionKey> it = selector.selectedKeys().iterator();// Get an iterator over the set of
																						// selected keys
			// 在被选择的set中遍历全部的key
			while (it.hasNext()) {
	 ...
	 * @param key
	 *
	 *            A SelectionKey object associated with a channel determined by the
